...
contract('Liquidator', accounts => {
	const [rUSD, wRWAX] = ['rUSD', 'wRWAX'].map(toBytes32);
	...
	const updateRatesWithDefaults = async () => {
		await updateRWAXPrice('6');
	};

	const updateRWAXPrice = async rate => {
		await updateAggregatorRates(exchangeRates, circuitBreaker, [wRWAX], [rate].map(toUnit));
	...
	describe('Default settings', () => {
		...
		it('liquidation penalty ', async () => {
			...
			assert.bnEqual(liquidationPenalty, RWAX_LIQUIDATION_PENALTY);
		});
	...
	describe('with issuanceRatio of 0.125', () => {
		...
		describe('calculateAmountToFixCollateral', () => {
			...
			describe('given target ratio of 800%, collateral of $600, debt of $300', () => {
				...
				describe('given liquidation penalty is 10%', () => {
					it('calculates rUSD to fix ratio from 200%, with $600 wRWAX collateral and $300 debt', async () => {
						const expectedAmount = toUnit('260.869565217391304347');

						// amount of debt to redeem to fix
						const rusdToLiquidate = await liquidator.calculateAmountToFixCollateral(
							debtBefore,
							collateralBefore,
							penalty
						);

						assert.bnEqual(rusdToLiquidate, expectedAmount);

						// check expected amount fixes c-ratio to 800%
						const debtAfter = debtBefore.sub(rusdToLiquidate);
						const collateralAfterMinusPenalty = collateralBefore.sub(
							multiplyDecimal(rusdToLiquidate, toUnit('1').add(penalty))
						);
					...
					it('calculates rUSD to fix ratio from 300%, with $600 wRWAX collateral and $200 debt', async () => {
		...
		describe('when Alice calls liquidateSelf', () => {
			...
			describe('when Alice is undercollateralized', () => {
				beforeEach(async () => {
					...
					// Alice issues rUSD $600
					await rwaone.transfer(alice, toUnit('800'), { from: owner });
					await rwaone.issueMaxRwas({ from: alice });

					// Bob issues rUSD $6000
					await rwaone.transfer(bob, toUnit('8000'), { from: owner });
					await rwaone.issueMaxRwas({ from: bob });

					// Drop wRWAX value to $1 (Collateral worth $800 after)
				...
				describe('when Alice issuance ratio is fixed as wRWAX price increases', () => {
					beforeEach(async () => {
						await updateRWAXPrice('6');
				...
				describe('when Alice c-ratio is above the liquidation ratio and attempts to self liquidate', () => {
					...
					it('then Alice still has 800 wRWAX', async () => {
						assert.bnEqual(await rwaone.collateral(alice), toUnit('800'));
					});
				});
				describe('given Alice has $600 Debt, $800 worth of wRWAX Collateral and c-ratio at 133.33%', () => {
					describe('when Alice calls self liquidate', () => {
						...
						beforeEach(async () => {
							...
							aliceDebtShareBefore = await rwaoneDebtShare.balanceOf(alice);
							aliceDebtValueBefore = await rwaone.debtBalanceOf(alice, rUSD);

							// Record Bobs state
							bobDebtValueBefore = await rwaone.debtBalanceOf(bob, rUSD);
						...
						it('it succeeds and the ratio is fixed', async () => {
							...
							assert.isTrue((await rwaoneDebtShare.balanceOf(alice)).lt(aliceDebtShareBefore));
							...
							// Check that the redeemed wRWAX is sent to the LiquidatorRewards contract
							...
							// Make sure the other staker, Bob, gets the redeemed wRWAX bonus.
							const bobDebtValueAfter = await rwaone.debtBalanceOf(bob, rUSD);
				...
				describe('with some wRWAX in escrow', () => {
					let escrowBalance;
					beforeEach(async () => {
				...
				describe('with only escrowed wRWAX', () => {
					let escrowBalanceBefore;
					beforeEach(async () => {
						await setLiquidRWAXBalance(alice, 0);
		...
		describe('when anyone calls liquidateDelinquentAccount on alice', () => {
			...
			describe('when Alice is undercollateralized', () => {
				...
				it('and liquidation penalty is 10%', async () => {
					assert.bnEqual(await liquidator.liquidationPenalty(), RWAX_LIQUIDATION_PENALTY);
				});
				...
				it('if not enough wRWAX to cover flag reward flagAccountForLiquidation reverts', async () => {
					await setLiquidRWAXBalance(alice, toUnit(1));
					await updateRWAXPrice('6');
					await rwaone.issueMaxRwas({ from: alice });
					await updateRWAXPrice('1');
				...
				describe('when Bob flags Alice for liquidation', () => {
					...
					describe('when deadline has passed and Alice issuance ratio is fixed as wRWAX price increases', () => {
						beforeEach(async () => {
							...
							await updateRWAXPrice(toUnit('6'));
					...
					describe('given Alice issuance ratio is higher than the liquidation ratio', () => {
						...
						it('if not enough wRWAX to cover flag reward isLiquidationOpen returns false', async () => {
							await setLiquidRWAXBalance(alice, toUnit(1));
							await updateRWAXPrice('6');
							await rwaone.issueMaxRwas({ from: alice });
							await updateRWAXPrice('1');
						...
						it('ignores RwaoneEscrow balance', async () => {
							await setLiquidRWAXBalance(alice, 1);
							...
							// cause bad c-ratio
							await updateRWAXPrice('1000');
							await rwaone.issueMaxRwas({ from: alice });
							await updateRWAXPrice('0.1');
							// should be false
							assert.isFalse(await liquidator.isLiquidationOpen(alice, false));
							// cannot flag the account
							await assert.revert(
					...
					describe('when the price of wRWAX increases', () => {
					...
					describe('given the liquidation deadline has passed ', () => {
						...
						describe('when Alice c-ratio is above the liquidation Ratio and Bob liquidates alice', () => {
							...
							it('then Alice still has 600 SDS', async () => {
								assert.bnEqual(await rwaoneDebtShare.balanceOf(alice), toUnit('600'));
							});
							it('then Alice still has 800 wRWAX', async () => {
								assert.bnEqual(await rwaone.collateral(alice), toUnit('800'));
							});
						...
						describe('when Alice burnRwasToTarget to fix her c-ratio ', () => {
							let burnTransaction;
							beforeEach(async () => {
								await updateRWAXPrice('1');
								burnTransaction = await rwaone.burnRwasToTarget({ from: alice });
							});
						...
						describe('when Alice burnRwas and her c-ratio is still below issuance ratio', () => {
							let aliceDebtBalance;
							let amountToBurn;
							beforeEach(async () => {
								await updateRWAXPrice('1');
								aliceDebtBalance = await rwaoneDebtShare.balanceOf(alice);
								amountToBurn = toUnit('10');
								await rwaone.burnRwas(amountToBurn, { from: alice });
							});
							it('then alice debt balance is less amountToBurn', async () => {
								assert.bnEqual(
									await rwaoneDebtShare.balanceOf(alice),
						...
						describe('when Alice burnRwas and her c-ratio is above issuance ratio', () => {
							let aliceDebtBalance;
							let amountToBurn;
							beforeEach(async () => {
								await updateRWAXPrice('1');
								...
								const maxIssuableRwas = await rwaone.maxIssuableRwas(alice);
								amountToBurn = aliceDebtBalance.sub(maxIssuableRwas).abs();
						...
						describe('when Alice burns all her debt to fix her c-ratio', () => {
							...
							beforeEach(async () => {
								...
								burnTransaction = await rwaone.burnRwas(aliceDebtBalance, { from: alice });
							});
							it('then alice has no more debt', async () => {
								assert.bnEqual(toUnit(0), await rwaoneDebtShare.balanceOf(alice));
							});
							it('then AccountRemovedFromLiquidation event is emitted', async () => {
								const logs = artifacts
									.require('Liquidator')
						...
						describe('when Alice does not fix her c-ratio ', () => {
							...
							describe('given Alice has $600 Debt, $800 worth of wRWAX Collateral and c-ratio at 133.33%', () => {
								describe('when bob calls liquidateDelinquentAccount on Alice', () => {
									...
									it('then Bob can liquidate Alice once fixing the c-ratio', async () => {
										...
										// check Alice has their debt share and collateral reduced
										assert.isTrue(
											(await rwaoneDebtShare.balanceOf(alice)).lt(aliceDebtShareBefore)
										);
										assert.isTrue((await rwaone.collateral(alice)).lt(aliceCollateralBefore));
							...
							describe('with only escrowed wRWAX', () => {
								...
								beforeEach(async () => {
									...
									// set up only escrow, no liquid wRWAX
									await setLiquidRWAXBalance(alice, 0);
									escrowBefore = await createEscrowEntries(alice, toUnit('1'), 100);

									// set up liquidation
									await updateRWAXPrice('8');
									assert.bnEqual(await systemSettings.issuanceRatio(), toUnit('0.125')); // 800% target c-ratio

									await rwaone.issueRwas(toUnit('100'), { from: alice }); // 800% c-ratio
									await updateRWAXPrice('1.4'); // price dumps
									assert.bnClose(
								...
								it('escrow balance is used for liquidation (partial)', async () => {
									const debtBefore = await rwaone.debtBalanceOf(alice, rUSD);
									const debtSharesBefore = await rwaoneDebtShare.balanceOf(alice);
									const totalDebtSharesBefore = await rwaoneDebtShare.totalSupply();
									const totalDebt = await rwaone.totalIssuedRwas(rUSD);
									...
									const debtAfter = await rwaone.debtBalanceOf(alice, rUSD);
									...
									const debtSharesAfter = await rwaoneDebtShare.balanceOf(alice);
									const debtDelta =
									...
									const res = toBN(dividend)
										.mul(toUnit('1.4')) // dividend * price of wRWAX ($1.40)
										.div(toBN(divisor));
							...
							describe('with some liquid and some escrowed', () => {
								...
								beforeEach(async () => {
									...
									await setLiquidRWAXBalance(alice, liquidBefore);
									// set up liquidation
									await updateRWAXPrice('6');
									await rwaone.issueMaxRwas({ from: alice });
									await updateRWAXPrice('1');
									...
									await fastForward((await liquidator.liquidationDelay()) + 100);
									await updateRWAXPrice('1');
									// add some escrow (200 wRWAX)
									// this is done now so that debt amount is determined by previous issueMaxRwas
								...
								it('if liquid is not enough, escrow is used for liquidation (partial)', async () => {
									await updateRWAXPrice('0.5');
									// add 90 more wRWAX in escrow (collateral value as with wRWAX @ 1, but with twice as much wRWAX
							...
							describe('last escrow entry remainder is added as new entry', () => {
								...
								beforeEach(async () => {
									...
									await fastForward((await liquidator.liquidationDelay()) + 100);
									await updateRWAXPrice('0.5');
									// add some escrow (200 wRWAX) as one entry
									// this is done now so that debt amount is determined by previous issueMaxRwas
		...
		describe('Given Alice has wRWAX and never issued any debt', () => {
			beforeEach(async () => {
		...
		describe('When Davids collateral value is less than debt issued + penalty', () => {
			...
			beforeEach(async () => {
				...
				// David issues rUSD $600
				await rwaone.transfer(david, toUnit('800'), { from: owner });
				await rwaone.issueMaxRwas({ from: david });

				// Drop wRWAX value to $0.1 (Collateral worth $80)
				await updateRWAXPrice('0.1');
			});
			it('then his collateral ratio should be greater than 1 (more debt than collateral)', async () => {
				...
				davidDebtBefore = await rwaoneDebtShare.balanceOf(david);
			...
			describe('when Bob flags and tries to liquidate David', () => {
				beforeEach(async () => {
					...
					// Drop wRWAX value to $0.1 after update rates resets to default
					await updateRWAXPrice('0.1');

					// ensure Bob has enough rUSD
				...
				describe('when the wRWAX rate is stale', () => {
					beforeEach(async () => {
				...
				describe('when Bob liquidates all of davids collateral', async () => {
					...
					it('then David should still have debt owing', async () => {
						const davidDebt = await rwaoneDebtShare.balanceOf(david);
