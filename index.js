...
const getFuturesMarkets = ({
	...
	return futuresMarkets.map(futuresMarket => {
		 ...
		 * Unfortunately due to some historical reasons 'rBTC', 'rETH' and 'sLINK' does not follow this format
		 * We adjust for that here.
		 */
		const marketsWithIncorrectAssetKey = ['rBTC', 'rETH', 'sLINK'];
...
const getPerpsMarkets = ({
	...
	return perpsMarkets.map(perpsMarket => {
		/**
		 * We expect the asset key to not start with an 's'. ie. AVAX rather than sAVAX
		 * Unfortunately due to some historical reasons 'rBTC' and 'rETH' does not follow this format
		 * We adjust for that here.
		 */
		const marketsWithIncorrectAssetKey = ['rBTC', 'rETH'];
